...
function KiranaPage() {
  ...
  // Exclude fresh categories (fruits, vegetables, milk/dairy, eggs/meat)
  const kiranaCats = (cats || []).filter((c: any) => {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-10">
        <div className="mb-6 flex items-end justify-between">
          <h1 className="text-2xl font-bold md:text-3xl">Kirana essentials</h1>
          <p className="text-sm text-muted-foreground">All categories except fruits & vegetables</p>
        </div>

        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          {kiranaCats.map((c: any) => (
