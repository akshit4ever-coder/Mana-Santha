...
export const resetPasswordByPhone = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      // List users (small projects) and find matching synthetic email
      const { data: listData, error: listErr } = await supabaseAdmin.auth.admin.listUsers();
      if (listErr) {
        console.error(listErr);
        return { success: false, error: listErr.message || String(listErr) };
      }

      const found = (listData?.users || []).find((u: any) => (u.email || "").toLowerCase() === syntheticEmail.toLowerCase());
      if (!found) {
        return { success: false, error: "No account found for this phone number." };
      }

      const metaName = (found.user_metadata && (found.user_metadata.full_name || found.user_metadata.name)) || "";
      if (metaName.trim().toLowerCase() !== String(data.fullName).trim().toLowerCase()) {
        return { success: false, error: "Provided name does not match our records." };
      }
      ...
      if (updErr) {
        ...
        return { success: false, error: updErr.message || String(updErr) };
      }
