    ...
    <section className="mt-3 px-1 md:px-0">
      <div className="container mx-auto">
        <div className="grid grid-cols-2 gap-2 px-0 md:grid-cols-4 md:gap-3">
          {items.map((item) => (
            <div key={item.title} className="flex items-start gap-2 sm:gap-3 rounded-lg bg-[#FFF9EC] px-2.5 py-2 sm:py-3 shadow-sm md:px-3" style={{ width: '100%' }}>
              <div className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-lg bg-[#F8F3E8] text-lg sm:text-xl font-medium text-[#176B3A]">{item.icon}</div>
              <div>
                <div className="text-sm font-semibold text-[#173522]">{item.title}</div>
                <div className="mt-1 text-xs sm:text-sm text-[#556955]">{item.subtitle}</div>
              </div>
            </div>
          ))}
        </div>
