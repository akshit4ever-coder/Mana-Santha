	...
	module.exports = class MimeNode {
		...
		*/
		setEnvelope(envelope) {
			...
			[
				...
				if (envelope[key]) this._convertAddresses(this._parseEnvelopeAddresses(envelope[key]), this._envelope.to, seenRecipients);
			});
		...
		getEnvelope() {
			...
			this._headers.forEach((header) => {
				...
				if (header.key === "From" || !envelope.from && ["Reply-To", "Sender"].includes(header.key)) {
					...
					"Cc",
					"Bcc"
				].includes(header.key)) this._convertAddresses(this._parseAddresses(header.value), envelope.to, seenRecipients);
			});
		...
		*/
		_encodeAddressName(name) {
			if (!/^[\w ]*$/.test(name)) {
				if (/^[\x20-\x7e]*$/.test(name)) return mimeFuncs.quoteString(name);
