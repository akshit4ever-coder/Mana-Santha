...
import { B as EyeOff, R as Funnel, S as Plus, b as Search, c as Trash2, g as ShoppingBag, j as LoaderCircle, o as TriangleAlert, t as X, w as Pencil, x as RotateCcw, z as Eye } from "../_libs/lucide-react.mjs";
...
import { a as moveProductImage, i as deleteProductImage, n as PLACEHOLDER_IMAGE, r as compressImageFile, s as uploadProductImage, t as ALLOWED_IMAGE_TYPES } from "./product-storage-C7t5u44h.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/admin.products-DAQT6rjJ.js
...
function AdminProducts() {
	...
	const save = useMutation({
		mutationFn: async (p) => {
			...
			if (!productId) throw new Error("Could not resolve the product id.");
			const variants = (p.product_variants ?? []).filter((variant) => variant?.name?.trim()).map((variant) => ({
				product_id: productId,
				name: variant.name.trim(),
				sku: variant.sku?.trim() || null,
				price: Number(variant.price ?? p.price ?? 0),
				mrp: Number(variant.mrp ?? p.mrp ?? 0),
				stock: Number(variant.stock ?? 0),
				unit: variant.unit || p.unit || null,
...
function ProductForm({ initial, categories, onSave, pending }) {
	...
	const { data: existingVariants = [] } = useQuery({
		queryKey: ["product-variants", initial?.id],
		queryFn: async () => {
			if (!initial?.id) return [];
			const { data, error } = await supabase.from("product_variants").select("*").eq("product_id", initial.id).order("sort_order");
			if (error) throw error;
			return data ?? [];
		},
		enabled: !!initial?.id
	...
	(0, import_react.useEffect)(() => {
		if (initial?.id) set("product_variants", existingVariants);
	}, [initial?.id, existingVariants]);
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("form", {
		onSubmit: async (e) => {
			...
			if (!trimmedName || !trimmedSlug || !p.price || !p.mrp) {
			...
			try {
				...
				if (imageFile) {
					setIsUploading(true);
					finalImageUrl = await uploadProductImage(await compressImageFile(imageFile), trimmedName);
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Label, {
						...
						variant: "outline",
						size: "sm",
						onClick: () => set("product_variants", [...p.product_variants ?? [], {
							name: "",
							price: p.price ?? 0,
							mrp: p.mrp ?? 0,
							stock: 0,
					...
					className: "space-y-2",
					children: (p.product_variants ?? []).map((variant, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "grid grid-cols-[1.2fr_0.8fr_0.6fr_auto] gap-2 rounded-md border bg-background p-2",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Input, {
								placeholder: "Variant name",
								value: variant.name ?? "",
								onChange: (e) => set("product_variants", (p.product_variants ?? []).map((item, itemIndex) => itemIndex === index ? {
									...item,
									name: e.target.value
								} : item))
							}),
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
								...
								variant: "ghost",
								size: "sm",
								onClick: () => set("product_variants", (p.product_variants ?? []).filter((_, itemIndex) => itemIndex !== index)),
								children: "Remove"
							})
						]
					}, `${variant.id ?? index}`))
