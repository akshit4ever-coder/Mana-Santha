...
//#region node_modules/.nitro/vite/services/ssr/assets/admin.products-CQmOlXV4.js
		...
		onSubmit: async (e) => {
			...
			try {
				...
				for (let i = 0; i < variants.length; i++) {
					...
					if (file) {
						const url = await uploadProductImage(await compressImageFile(file), `${trimmedName}-${variants[i].name || i}`);
