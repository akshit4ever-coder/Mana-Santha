...
import { E as Minus, L as Heart, S as Plus, a as Truck, j as LoaderCircle, v as ShieldCheck } from "../_libs/lucide-react.mjs";
import { d as useWishlist, l as useToggleWishlist, n as useAddToCart, o as useProduct, r as useCart, s as useProducts, t as Header, u as useUpdateCartQty } from "./Header-CA9hksBN.mjs";
...
import { t as Route } from "./product._slug-Co-y_Hwx.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/product._slug-CyWSAG3A.js
...
function ProductPage() {
	...
	const firstVariant = product?.product_variants?.find((variant) => variant.is_active !== false) ?? null;
	const [selectedVariantId, setSelectedVariantId] = (0, import_react.useState)(firstVariant?.id ?? null);
	const selectedVariant = product?.product_variants?.find((variant) => variant.id === selectedVariantId) ?? firstVariant ?? null;
	const item = cart?.find((c) => c.product_id === product?.id && (selectedVariant ? c.variant_id === selectedVariant.id : !c.variant_id));
	const isWish = wl?.some((w) => w.product_id === product?.id);
	const price = product ? selectedVariant ? Number(selectedVariant.price ?? product.price) : Number(product.price) : 0;
	const displayMrp = product ? selectedVariant ? Number(selectedVariant.mrp ?? product.mrp) : Number(product.mrp) : 0;
	const pct = product ? discountPct(displayMrp, price) : 0;
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("img", {
								src: product.image_url || "/assets/images/product-placeholder.png",
								alt: product.name,
								loading: "lazy",
								decoding: "async",
							...
							product.product_variants?.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "mt-4",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									...
									children: product.product_variants.map((variant) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
										type: "button",
										onClick: () => setSelectedVariantId(variant.id),
										className: `rounded-full border px-3 py-2 text-sm ${selectedVariantId === variant.id ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background text-muted-foreground"}`,
								...
								children: [item ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Button, {
											...
											variant: "ghost",
											className: "h-10 w-10 rounded-full text-primary-foreground hover:bg-primary-glow/40 hover:text-primary-foreground",
											disabled: item.quantity >= product.max_qty || item.quantity >= product.stock,
											onClick: () => upd.mutate({
												id: item.id,
												quantity: item.quantity + 1
											}),
									...
									children: (selectedVariant?.stock ?? product.stock) <= 0 ? "Out of stock" : "Add to Cart"
